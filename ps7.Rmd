...
for (i in 8:length(c1$age)){
  ...
  c2$alive[i] = c2$alive[i-1] - (c2$alive[i-1]*c2$mort[i])
}

#calculate frailty for each age group 
c1$frail = rep(0,length(c1$age))
c1$frail[8] = mean(p1$frailty[1:round(c1$alive[8], 0)])
c2$frail = rep(0,length(c2$age))
c2$frail[8] = mean(p2$frailty[1:round(c2$alive[8], 0)])

for (i in 9:length(c1$age)){
 c1$frail[i] = mean(p1$frailty[round(c1$alive[i-1],0):round(c1$alive[i], 0)])
 c2$frail[i] = mean(p1$frailty[round(c2$alive[i-1],0):round(c2$alive[i], 0)])
...
pi0 = 0.95
c1$pi = (pi0*c1$alive)/ (pi0*c1$alive + (1-pi0)*c2$alive) #this is eq2 from 
...
ggplot(c2) + geom_point(aes(x=age, y = log(mort))) + 
  geom_point(aes(x=age, y = log(c1$meanmort)))
```



# HOMEWORK-RELATED NOTES FROM CLASS ON DEC 7: 
